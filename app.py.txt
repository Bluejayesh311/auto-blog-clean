...
# Read secrets from environment variables — NEVER hardcode API keys here
openai.api_key = os.getenv('OPENAI_API_KEY')
WP_URL = os.getenv('WP_URL')          # e.g. https://yourwordpresssite.com/wp-json/wp/v2/posts
...
def post_to_wordpress(title, content):
    ...
    post_data = {
        'title': title,
        'content': content,
        'status': 'publish'  # change to 'draft' for testing
    }
    try:
